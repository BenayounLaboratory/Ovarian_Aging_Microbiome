...
# Public ovarian aging bulk mRNA-seq analysis - CRA003645
# Pre-processing and DESeq2
# Compare gene expression patterns between CRA003645 and FMT ovarian bulk mRNA-seq datasets
###############################################

##### read in count matrix #####

my.ovary.counts1 <- read.table("./MM_CRA003645_ovary_aging_mRNAseq_featureCounts_output.txt",
                               sep="\t", header=T)

my.ovary.counts2 <- my.ovary.counts1[,c(1, 7, 11, 12, 16:19)]
...
my.Age <- c(rep("3M", 4), rep("12M", 3))

rownames(my.ovary.counts) <- my.ovary.counts$GeneName

# get the genes with no reads out
my.good <- which(apply(my.ovary.counts[,-1]>0, 1, sum) >= 4) # see deseq2 vignette, need to remove too low genes
my.filtered.matrix <- my.ovary.counts[my.good,-1] # 14412  genes
...
write.table(my.filtered.sva, file = paste(Sys.Date(),"MM_CRA003645_STAR_postSVA_Counts.txt", sep ="_"), sep = "\t" , row.names = T, quote=F)

################################################################################
# 2. DESeq2 on cleaned data

my.outprefix <- paste(Sys.Date(),"MM_CRA003645_DESeq2_Analysis_STAR_post_SVA",sep="_")
...
my.colors[grep("1Y",colnames(tissue.cts))] <- "deeppink4"
...
my.outprefix <- paste(Sys.Date(),"MM_CRA003645_DESeq2_aging",sep="_")

res.Age <- results(dds.deseq, contrast = c("age", "12M", "3M"))       # OF / YF
...
save(res.Age, file = paste(Sys.Date(),"MM_CRA003645_Age_Ovaries_Age.RData", sep ="_"))
