...
my.results.16  <- read_qza("./aldex2_AC_males_cohort16/differentials.qza")$data
my.results.17  <- read_qza("./aldex2_AC_males_cohort17/differentials.qza")$data
my.results.22  <- read_qza("./aldex2_AC_males_cohort22/differentials.qza")$data

my.taxonomy.16 <- read_qza("../../5_Taxonomy/silva_MM_AC16_males_taxonomy.qza")$data
my.taxonomy.17 <- read_qza("../../5_Taxonomy/silva_MM_AC17_males_taxonomy.qza")$data
my.taxonomy.22 <- read_qza("../../5_Taxonomy/silva_MM_AC22_males_taxonomy.qza")$data

my.taxonomy.combined <- rbind(my.taxonomy.16, my.taxonomy.17, my.taxonomy.22)
...
common_features <- Reduce(intersect, list(my.results.16$Feature.ID, my.results.17$Feature.ID, 
                                          my.results.22$Feature.ID))

length(common_features)     #387
...
combined.results.1 <- cbind(my.results.16.filt[c(1,5)], my.results.17.filt[5], my.results.22.filt[5])
colnames(combined.results.1) <- c("Feature.ID", "diff.btw.16", "diff.btw.17", "diff.btw.22")

# Calculate the sign for each of the last four columns
signs <- sign(combined.results.1[, 2:4])

# Sum the signs for each row
row_sums <- rowSums(signs)

# Identify rows where the absolute sum is 4 (all signs are the same)
all_negative_rows <- rowSums(combined.results.1[, 2:4] < 0) == 3
all_positive_rows <- rowSums(combined.results.1[, 2:4] > 0) == 3
...
combined.results.2 <- cbind(my.results.16.filt[c(1,11)], my.results.17.filt[11], my.results.22.filt[11])
colnames(combined.results.2) <- c("Feature.ID", "wi.ep.16", "wi.ep.17", "wi.ep.22")
...
count_significant.neg <- sum(result_df.neg$BH_Corrected_P_Value < 0.05)     # 51
...
ggplot(significant_effects_unique, aes(x = reorder(Feature.ID, average_effect), y = average_effect, size = minus_log10_pvalue)) +
  ...
  labs(title = "Significant Features - AC males", 
       x = "Features", 
       y = "Effect", 
       size = "-log10(P-value)", 
       color = "Effect")

ggsave("MM_AC_males_Differential_bubble_plot.pdf", height=15, width=7, device="pdf", useDingbats=F)
